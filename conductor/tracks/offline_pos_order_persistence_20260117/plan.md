...
- [ ] Task: Update `lib/db.ts` to include `orders` table.
    - [ ] Add `orders: 'id, created_at, status, total_amount'` to the `.stores()` definition.
    - [ ] Define `id` as primary key for orders, with client-side UUID generation.
    - [ ] Define predefined values for `status` (e.g., 'pending', 'completed', 'cancelled').
- [ ] Task: Update `lib/db.ts` to include `order_items` table.
    - [ ] Add `order_items: 'id, order_id, lot_id, quantity, price_at_sale'` to the `.stores()` definition.
- [ ] Task: Update `lib/db.ts` to include `customers` table.
    - [ ] Add `customers: 'id, name, email, phone'` to the `.stores()` definition.
- [ ] Task: Conductor - User Manual Verification 'Dexie Schema Update' (Protocol in workflow.md)
